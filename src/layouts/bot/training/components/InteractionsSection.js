...
const InteractionsSection = ({
  ...
  const isAddDisabled = isAddingInteraction || (!editMode && !createdBotId);
  const inputsDisabled = (!editMode && !createdBotId);
  ...
  return (
    <Card sx={{ p: 3, mb: 4 }}>
      ...
      {/* Mover Nombre y Descripción arriba para crear el bot antes de añadir interacciones */}
      <SoftBox mt={2} mb={1}>
        <SoftTypography variant="subtitle2" fontWeight="bold" mb={1} color="dark">Nombre y Descripción</SoftTypography>
        ...
        {/* Mostrar botón Crear Bot aquí cuando se está en modo creación */}
        {!editMode && (
          <SoftBox display="flex" justifyContent="flex-end" sx={{ mb: 2 }}>
            <SoftButton variant="gradient" color="success" onClick={handleCreateBot} disabled={creatingBot || createdBotId} sx={{ minWidth: 140 }}>
